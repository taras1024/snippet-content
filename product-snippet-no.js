...
const translations = {
  ...
  analyticalConstituents: "Analytiske bestanddeler",
  nutritionalAdditives: "Ernæringsmessige tilsetningsstoffer",
  feedingGuide: "Fôringsguide",
};
/*
 *Snippet GUI start
 */
...
function changePage() {
  ...
  const aliasCheckBox = document
    .getElementById("snippetIframe")
    .contentWindow.document.querySelector(
      'input[data-drupal-selector="edit-path-0-pathauto"]'
    );
  if (aliasCheckBox) {
...
async function setLanguageSelect() {
  const node = document
    .getElementById("snippetIframe")
    .contentWindow.document.getElementById("edit-langcode-wrapper");
...
function showFoundedOrNotFoundedValue(node, values, addedOrNotAddedColor) {
  ...
  if (
    node.id === "edit-field-product-range-wrapper" ||
...
function ingredientsAndNutritionFormatter() {
  ...
  return [
    ...
    }</strong></p><p>${copydeckIngredients.replace(/\n/g, "<br>")}</p>`,
    `<p><strong>${
      translations.analyticalConstituents
    }</strong></p><p>${copydeckNutritionAnalyticalConstituents.replace(
      /\n/g,
      "<br>"
    )}</p>
		 <p><strong>${
       translations.nutritionalAdditives
     }</strong></p><p>${copydeckIngredientsNutritionalAdditives.replace(
      /\n/g,
      "<br>"
    )}</p>`,
  ];
...
function feedingGuideFormatter() {
  ...
  return [
    `<p><strong>${
      translations.feedingGuide
    }</strong></p><p>${copydeckFeedingGuide
      .replace(/["♥]/gi, "")
      .replace(/<\/?[^>]+(>|$)/g, "")
      .replace(/\n/g, "<br>")}</p>`,
  ];
